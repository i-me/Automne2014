...
public class ProgArray2 {
	...
	public static void main(String[] args) 
	{
		 ...
		 */
		String sChoixMenu[]={"Moyenne/Étendu", "Tableau fréquence des notes", "Recommencer", "Quitter"}, sSaisie="", sSortie="";//sChoixMenu... choix du menu, sSaisie, sert à la saisie des notes de l'user, sSortie, chaine vide pour construire une sortie
		char cCase1[]={'|','*','*','*','*','*','*','*','*','*','*','|'}, cCase2[]={'|','*','*','*','*','*','*','*','*','*','*','|'};
		String sCase3[]={"*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*","*"};
		String sMiniChaine="";//Ce sont des variables necessaires à la construction du tableau des valeurs
		boolean err=false;//Va être utile dans les boucles. 
		do
		{
			switch(iChoixMenu)
			{
			...
			case 1://Tableau de fréquence des notes
				...
				{/*On construit le tableau de fréquences (f*cking alignement des cases. I give up)*/
			...
			case 2: case -1://On recommence la saisie des notes
				...
				dNote100=new double[20];//Il y a 20 notes saisies; Contient les notes de dSaisie sur 72 -> transformés sur 100;	
				for(int i=0;i<20;i++)//On a besoin de 20 entrées 
				{
					dSaisie=-1;
					...
					{
						dNoteMax=(dNoteMax<dSaisie||dNoteMax<0)?dNoteMax=dSaisie:dNoteMax; // Si ça marche. Voir avec examen pour prof. 
						dNoteMin=(dNoteMin>dSaisie||dNoteMin<0)?dNoteMin=dSaisie:dNoteMin;
						dTotalNote+=dSaisie;
						if(dSaisie==100)//Pour pas faire d'erreur de traitement
							dSaisie-=1;
						iClass[((int)dSaisie/10)]++;//On enregistre la saisie dans la bonne entrée du tableau;
